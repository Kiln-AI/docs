...
To create a fine-tune targeting tool calling, you must generate a training set. The samples in the training set must have had the same set of tools available to them when they were run, as the model you are training. This doesn’t mean every tool needs to be called in every training sample. Only that every tool was available to be called.
...
3.  Select the set of tools which the model should learn to call. <br>
...
4.  Click “Add Fine-Tuning Data” to launch Kiln's synthetic data generation tool <br>
...
{% hint style="info" %}
You must select a base model which supports tool calling. Kiln will automatically disable tool calling if not supporte
{% endhint %}
