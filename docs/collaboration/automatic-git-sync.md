...
| ---------------------------------- | -------------------------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------- |
| **Ease of Use**                    | ❌ Requires terminal commands, dev tools, manual sync, confilict resolution | :white\_check\_mark: Log in and go   | :white\_check\_mark: Connect and go                   |
...
* **PMs / Subject Matter Experts / Designers:** connect an existing Kiln project in a repository in seconds via Kiln's UI. Any work you do (ratings, specs, data annotation) is atomically instantly synced to Git, without any manual intervention. They never see a terminal, or have to learn git.
...
* Create a Kiln project using the Kiln UI (or choose and existing project)
* Create an empty remote repository on GitHub, or the provider of your choice
* Push the new project to your remote repository. Follow your providers instructions but typically something like `git init -b main && git add . && git commit -m "initial commit" && git remote add origin && git push -u origin main`
* Ensure your team has read/write access to the repository using the providers user mangement

{% hint style="info" %}
**Picking a Git Host**

Kiln works with any remote Git host. Most orgs already have a preferred provider, but there are small difference between how they work with Kiln.&#x20;
...
* **GitLab**: one click link to generate an personal access token. Easy even for non technical users. May require approvals based on organization policies.
...
* Navigate to **Settings > Manage Projects > Import Project > Automatic Git Sync** in Kiln UI
...
* Copy the URL from the browsers, this is now a deep link to add this project on any team member's machine.
* Share the link with your team! If your using a provider other than GitHub or GitLab also share instructions on how to generate a personal access token.
...
* Follow in the app steps to connect the project
* Use it like any other project, except know it's automatically kept up to date with your team!

{% hint style="info" %}
This path can be used for any user, regardless of of technical background. The cloning, syncing, authorization, and all other details are managed automaticlly.
{% endhint %}
...
* Each project is cloned to it's own private and hidden repo for avoid conflicts
...
Kiln will work with ssh access URLs if your machine already has SSH setup with your git provider. Just paste the ssh URL starting with `git@` when setting up the repository.

However, non-devs are very unlikely to have ssh setup. If ssh fails, we'll suggest trying the https URL and oAuth/Token based authentication instead.&#x20;
