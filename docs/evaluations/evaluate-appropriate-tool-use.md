...
Appropriate Tool Use evals measure how well your model decides when to invoke a specific tool by generating test scenarios that include both situations where the tool should and shouldn't be called. This approach includes:

* Generating large eval datasets quickly using synthetic data generation
* Creating realistic scenarios that comprehensively test tool invocation behavior
...
This guide covers the tool-specific aspects of appropriate tool use evals. For the general eval workflow including setting up judges, adding human ratings, finding the ideal judge, and finding the ideal run configuration, see [Evaluations](./README.md).

The key differences for appropriate tool use evals are:

* [Creating the eval](#creating-an-appropriate-tool-use-eval) requires specifying tool use guidelines
* [Synthetic data generation](#generate-synthetic-test-data) automatically enables the tool being evaluated
* [Human ratings](#add-human-ratings) require checking the message trace to see tool invocation details
* [Judge](#finding-the-ideal-judge) examines the entire message trace to determine appropriate tool use 
* [Finding the ideal run configuration](#finding-the-ideal-run-configuration) involves ensuring the tool being evaluated is available in the run configuration you are evaluating
...
For more details on the general human rating workflow, see [Add Human Ratings](./README.md#add-human-ratings).
...
For detailed guidance on selecting judge algorithms (LLM as Judge vs G-Eval), models, and customizing evaluation steps, see [Add a Judge to your Eval](./README.md#add-a-judge-to-your-eval).

The judge will examine the full conversation trace to determine appropriate tool use, using your provided guidelines.

For guidance on comparing judges and finding the one that best aligns with human preferences, see [Finding the Ideal Judge](./README.md#finding-the-ideal-judge).

### Finding the Ideal Run Configuration

Once you have a judge set up, you can evaluate different configurations for running your task. Since appropriate tool use evals specifically test your model's tool calling behavior, you'll want to test configurations such as:
...
<figure><img src="../.gitbook/assets/Screenshot 2025-11-14 at 1.33.24 PM.png" alt="" width="375"><figcaption></figcaption></figure>

For detailed guidance on selecting and comparing task model options, see [Finding the Ideal Run Method](./README.md#finding-the-ideal-run-method).
