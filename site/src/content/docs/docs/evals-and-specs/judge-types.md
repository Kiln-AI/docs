---
title: "Judge Types"
description: "Score your evals with LLM judges, or with fast deterministic checks that cost nothing to run"
---
...
An LLM judge that decides "did the agent call `get_weather` before answering?" costs money on every run and can change its mind. A [Tool Call Check](/docs/evals-and-specs/judge-types/#tool-call-check) answers the same question for free, identically, every time.
...
| ---------- | ------ | -------------- |
| [LLM as Judge](/docs/evals-and-specs/judge-types/#llm-as-judge) | LLM Judge | A model grades the output against a rubric you write |
| [Code](/docs/evals-and-specs/judge-types/#code-beta) | Programmatic Judge | A custom Python `score()` function you write |
| [Tool Call Check](/docs/evals-and-specs/judge-types/#tool-call-check) | Programmatic Judge | The agent called the right tools, in the right order, with the right arguments |
| [Exact Match](/docs/evals-and-specs/judge-types/#exact-match) | Programmatic Judge | The output equals an expected value |
| [Pattern Match](/docs/evals-and-specs/judge-types/#pattern-match) | Programmatic Judge | The output matches (or doesn't match) a regular expression |
| [Contains](/docs/evals-and-specs/judge-types/#contains) | Programmatic Judge | The output contains (or omits) a substring |
| [Set Check](/docs/evals-and-specs/judge-types/#set-check) | Programmatic Judge | A set of values from the output matches an expected set |
| [Step Count Check](/docs/evals-and-specs/judge-types/#step-count-check) | Programmatic Judge | The agent finished within an expected number of steps |
...
1. **Is there exactly one right answer?** Use [Exact Match](/docs/evals-and-specs/judge-types/#exact-match) (or [Set Check](/docs/evals-and-specs/judge-types/#set-check) if the answer is a set of values).
2. **Can you state the rule as text matching?** Use [Contains](/docs/evals-and-specs/judge-types/#contains) or [Pattern Match](/docs/evals-and-specs/judge-types/#pattern-match). Good for format rules ("always ends with a citation", "never mentions a competitor").
3. **Is it about what the agent did, not what it said?** Use [Tool Call Check](/docs/evals-and-specs/judge-types/#tool-call-check) for which tools ran, or [Step Count Check](/docs/evals-and-specs/judge-types/#step-count-check) for how many steps it took.
4. **Is the rule real, but too complex for the above?** Use a [Code](/docs/evals-and-specs/judge-types/#code-beta) judge. It can also call an LLM for the subjective part, so you can filter a huge trace down in code and only pay for a judgement on what's left. See [Code Judges](/docs/evals-and-specs/code-judges/).
5. **Otherwise, use an** [**LLM as Judge**](/docs/evals-and-specs/judge-types/#llm-as-judge). Subjective quality needs a model.
...
Tool Call Check and Step Count Check always read the trace, so they don't offer this option. [Code](/docs/evals-and-specs/judge-types/#code-beta) judges receive the output and trace directly, and pick out what they need in Python.
...
* **Output to Check**: see [above](/docs/evals-and-specs/judge-types/#output-to-check).
